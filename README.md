...
보조 머신은 모든 요청을 메인 서버로 프록시합니다. 이때 `proxy_target_ip`와 두 머신의 통신 경로는 **VPN(Tailscale) IP**여야 합니다 — 메인 서버의 allowlist가 프록시의 출발지 IP를 검사하므로 LAN IP로 연결하면 `403`이 됩니다(부득이하면 메인의 `allow_cidr`에 해당 대역을 추가, 보안 수준은 내려감).

> 참고: 보조 머신은 본질적으로 "보조 기기 브라우저에서 메인으로 접속"의 자동화일 뿐입니다. 보조 머신에서도 직접 메인 IP(`http://100.x.x.x:18080/`)에 접속하면 같은 화면이 나오므로, 단일 머신 구성으로도 충분한 경우가 많습니다. 프록시는 LaunchAgent 자동 시작·로컬호스트 북마크 같은 편의가 필요할 때만 추천.

### HTTPS

이 서버는 의도적으로 HTTP로 동작합니다. Tailscale의 WireGuard로 종단간 암호화되어 있고, IP 자체가 공개망에서 접근 불가능한 mesh 사설 IP라서 추가 TLS의 실익이 적기 때문입니다. 정식 인증서가 필요하면 `tailscale cert` + Let's Encrypt를 사용할 수 있습니다.

### 백업

`backup.sh`를 cron에 등록해서 주간 자동 백업:
...
- **OAuth 토큰 만료** — Claude Code는 `/login`으로 발급받은 OAuth 토큰으로 인증하며, 이 토큰은 일정 기간 후 만료됩니다. 만료되면 모든 세션에서 동시에 `401 authentication_error`가 발생하고, 데스크탑에서 직접 `/login`을 다시 실행해야 합니다. 원격에서는 재로그인할 수 없으므로 데스크탑에 물리적으로 접근해야 합니다.
