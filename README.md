...
- 외출 중에도 맥미니에게 **새 작업을 시키고** 싶은 사람

항상 인터넷에 연결된 데스크탑 한 대가 Claude Code의 메인 머신이고, 이동 중에는 다른 기기에서 원격으로 모니터링하고 작업을 지시하는 워크플로우를 전제로 만들었습니다. 대시보드에서 시작한 세션은 `--remote-control` 플래그가 자동 적용되어 [claude.ai/code](https://claude.ai/code) 웹앱에서도 바로 접속할 수 있습니다.
