...
#
# 이 스크립트는 다음을 수행합니다:
# 1. 선택 설정 (API 키, trust dialog 등)
# 2. 세션 JSONL → HTML 변환 (convert_session.py)
# 3. Claude Code Stop hook 등록 (세션 종료 시 자동 변환)
# 4. LaunchAgent 등록 (서버 자동 시작 + 재시작)
# 5. 서버 시작 및 브라우저 열기
...
if [ -n "$ANTHROPIC_API_KEY" ]; then
    ...
    read -p "  Anthropic API 키 입력 (Enter로 건너뛰기): " INPUT_API_KEY
    if [ -n "$INPUT_API_KEY" ]; then
        ...
        if [ -n "$SHELL_PROFILE" ]; then
            ...
            echo "    영구 적용하려면 셸 설정 파일에 다음을 추가하세요:"
            echo "    export ANTHROPIC_API_KEY=\"${INPUT_API_KEY}\""
        fi

        # LaunchAgent에서도 API 키를 사용할 수 있도록 저장
...
# --- 1b. Trust dialog 스킵 ---
echo "📌 Trust dialog 스킵 (선택)"
echo "   대시보드에서 새 세션을 시작할 때 'trust this folder' 확인창이 뜨면"
echo "   세션이 자동으로 진행되지 않습니다. 이를 건너뛸 수 있습니다."
echo ""

NEEDS_TRUST_SETUP=false
...
if [ "$HAS_TRUST" = "yes" ] && [ "$HAS_SKIP" = "yes" ]; then
    ...
    read -p "  Trust dialog을 자동으로 건너뛸까요? (Y/n): " TRUST_CONFIRM
...
"
        ...
        # settings.json 설정 (아직 없을 수 있으므로 Step 2 전에 생성)
        if [ ! -f "$SETTINGS_FILE" ]; then
...
if command -v tailscale &>/dev/null; then
    ...
    if [ -n "$TS_IP" ]; then
        ...
        echo "    tailscale up 으로 연결한 후 외부에서 접속할 수 있습니다."
    fi
else
...
# API 키가 있으면 LaunchAgent에 환경변수로 포함
ENV_SECTION=""
...
if [ -n "$TS_IP_FINAL" ]; then
    echo "   외부 (VPN):     http://${TS_IP_FINAL}:${PORT}"
fi
if [ -n "$LOCAL_IP" ]; then
